...
app.get('/admin/vagas/nova', async (request, response) => {
    ...
    response.render('admin/novaVaga', { categorias })
})
...
app.get('/admin/vagas/editar/:id', async (request, response) => {
    ...
    response.render('admin/editarVaga', { categorias, vaga })
})
...
app.post('/admin/vagas/editar/:id', async (request, response) => {
    ...
    await db.run(
        `update vagas set categoria = ${categoria}, titulo = '${titulo}', descricao = '${descricao}' where id = ${
            request.params.id
        };`
    )
...
app.get('/admin/categorias/nova', async (request, response) => {
    response.render('admin/novaCategoria')
})
...
app.get('/admin/categorias/editar/:id', async (request, response) => {
    ...
    const categoria = await db.get(
        `select * from categorias where id = ${request.params.id}`
    )

    response.render('admin/editarCategoria', { categoria })
})
...
app.post('/admin/categorias/editar/:id', async (request, response) => {
    ...
    await db.run(
        `update categorias set categoria = '${categoria}' where id = ${
            request.params.id
        };`
    )
...
app.listen(port, err => {
    err
        ? console.log('Nao foi posivel iniciar o servido do Jobify.')
